...
export default function Computation() {
  ...
  const getStatusClass = (status: RequestStatus) => {
    switch (status) {
      case RequestStatus.Accepted: return 'bg-green-100 text-green-800';
            ...
            {requests.map((request) => (
              <Card key={request._id} className="p-4">
                <div className="flex flex-col gap-2">
                  <h3 className="text-[0.8rem] text-wrap">Request from {request.senderAddress}</h3>
                  <p className="text-sm text-gray-500">Amount: {request.amount} APT</p>
                  <p className={`text-sm px-2 py-1 rounded ${getStatusClass(request.status)}`}>

                    {request.status.toUpperCase()}
                  </p>
                  {dockerurl 
                    && request.status.toLowerCase() == "completed" &&  <p>
docker pull hemil36/hemil-docker-image && docker run -p 5000:5000 hemil-docker-image
                    </p>}
                  {
                    dockerurl 
                    && request.status.toLowerCase() == "completed" &&  <p>
docker pull hemil36/hemil-docker-image && docker run -p 5000:5000 hemil-docker-image
                    </p> && <Button onClick={() => {setPollingDialogOpen(true);startPolling();}} variant="outline">Start Polling</Button>
                  }
                  <button
                    onClick={() => {
      ...
      <div className="py-4">
        <div className="space-y-6">
          ...
          <div>
            ...
            <p className="text-sm">{selectedRequest.senderAddress}</p>
          </div>
          ...
          <div className="flex gap-4 justify-end">
            ...
            {
              actionLoading && <Loader2 className="animate-spin h-8 w-8 text-gray-300" />
            }
...
<Dialog open={pollingDialogOpen} onOpenChange={setPollingDialogOpen}>
        <DialogContent className="bg-gray-800 text-white p-6 rounded-lg max-w-md mx-auto">
          <DialogHeader>
            ...
            <DialogDescription className="text-sm text-gray-400">
              The system is polling for updates every 10 seconds. You can cancel this at any time.
            </DialogDescription>
          </DialogHeader>
